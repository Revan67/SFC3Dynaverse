...
|---|---|---|---|---|
| Campaign clock | Retail publishes five little-endian integers: turn `93528`, current year `9`, turns/year `10000`, milliseconds/turn `120000`, base year `56200`. The asynchronous form appends a zero byte; the synchronous reply instead prefixes a success byte. The turn-break snapshot is sent to two registered clock callbacks before subsequent character/news traffic. | `tCurrentTime::StreamOut` writes offsets `+0x0c,+0x10,+0x14,+0x18,+0x1c`. `tClockRelayS::AnnounceTurn` filters client registrations by their requested frequency, sends `tTurnAnnouncement` to clients first, and only then dispatches work to Database, Character, Map, AI, Goal, Mission Matcher, and Ship services. Those services RSVP asynchronously. `Time.gf` supplies `10000`, `120000`, and `56200`. | The original replacement used one timer per connection, ignored registration frequency, and settled auctions before publishing the clock. A single campaign coordinator now announces first, processes each settlement once globally, and notifies winners afterward. | **Corrected; awaiting stock-client live validation.** Automated tests cover both exact wire envelopes, registration parsing/frequency, retail phase ordering, rollover, restart stability, scheduling, and settlement boundaries. |
