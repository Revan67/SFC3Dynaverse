...
- Officer transfers are being lost because the updated slots are not surviving
  in the canonical serialized ship.
...
2. Make one canonical serialized `tShip` the source of truth for Supply Dock,
   Refit, Officers, channel-7 refresh, persistence, and relog. The three current
   facility defects are different symptoms of this shared boundary.
